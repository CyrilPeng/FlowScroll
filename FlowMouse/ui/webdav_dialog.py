...
class WebDAVSyncDialog(QDialog):
    ...
    def save_config(self):
        ...
        QMessageBox.information(self, "提示", "WebDAV 配置已暂存，请记得在主界面保存预设以持久化。")
        self.accept()
        ...
        try:
            ...
            with urllib.request.urlopen(req) as response:
                if response.status in (200, 201, 204):
                    QMessageBox.information(self, "成功", "配置已成功上传至 WebDAV 云端！")
                else:
                    QMessageBox.warning(self, "失败", f"上传失败，状态码: {response.status}")
    ...
    def download_config(self):
        ...
        try:
            ...
            QMessageBox.information(self, "成功", "配置已从云端下载成功！重启程序或重新加载预设生效。")
        except Exception as e:
            QMessageBox.critical(self, "错误", f"下载配置失败，可能云端尚无配置文件:\\n{str(e)}")
