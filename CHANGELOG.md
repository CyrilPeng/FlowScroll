...
### Changed
- `MainWindow` 业务逻辑抽取到 `ApplicationController`（`ui/app_controller.py`），主窗口仅负责 UI 层逻辑，线程生命周期、预设管理、更新检测等均委托给控制器
- `BUILTIN_PRESETS` 提取 `_PRESET_DEFAULTS` 共享默认值，各预设仅定义差异字段，消除四处重复
- `GlobalConfig.to_dict` / `to_dict_for_sync` 提取 `_to_dict_common` 公共方法，消除序列化逻辑的完全重复
- `ScrollEngine.run()` 主循环中的 `STATE_LOCK` 快照抽取为 `_snapshot_config()` 方法
- `test_smoke.py`（1500+ 行）按模块拆分为 `test_config.py`、`test_services.py`、`test_keyboard_webdav.py`
- `single_instance.py` 的哈希算法从 SHA-1 改为 SHA-256
- `i18n.py` 中对 `cfg` / `STATE_LOCK` 的导入改为函数内延迟导入，消除模块级循环依赖风险
- `main.py` 崩溃处理为非 Windows 平台添加 `traceback.print_exc()` 回退，避免依赖 `QApplication`
...
- 修复 WebDAV 保存配置时对 `http://` 明文连接无警告的问题，现在会弹出安全风险提示
- 修复 `overlay.py` 的 `show_preview` 使用 `geometry()` 而非 `availableGeometry()`，导致多显示器 + 不同 DPI 下预览偏移
- 修复 `autostart.py` 中 docstring 因编码错误产生的乱码
...
- 新增 `webdav.insecure_http_warning` 中英文本地化词条

### Removed
- 移除 `services/crypto.py` 死代码（XOR 伪加密，未被任何模块引用）
