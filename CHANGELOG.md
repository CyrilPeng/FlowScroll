...
### Changed
- `MainWindow` ??????? `ApplicationController`?`ui/app_controller.py`???????? UI ????????????????????????????
- `BUILTIN_PRESETS` ?? `_PRESET_DEFAULTS` ???????????????????????
- `GlobalConfig.to_dict` / `to_dict_for_sync` ?? `_to_dict_common` ?????????????????
- `ScrollEngine.run()` ????? `STATE_LOCK` ????? `_snapshot_config()` ??
- `test_smoke.py`?500+ ???????? `test_config.py`?`test_services.py`?`test_keyboard_webdav.py`
- `single_instance.py` ?????? SHA-1 ?? SHA-256
- `i18n.py` ?? `cfg` / `STATE_LOCK` ????????????????????????
- `main.py` ?????? Windows ???? `traceback.print_exc()` ??????? `QApplication`

### Fixed
- ?? `ScrollEngine` ? `_scroll_history` / `_mouse_pos_history` ????????`interrupt_inertia` ????????????????????/????????? `collections.deque` + ?? `_history_lock`????????????????`_prune_history` ?? `popleft()` ?? O(1) ??
- ?? UI ???? `cfg` ?????????????`tabs_builder`?`settings_window`?`dialogs`?`webdav_dialog` ?? `cfg.xxx = ...` ????? `set_config_attr()`?? `STATE_LOCK` ?????
- ?? `GlobalInputListener` ???????????????? `mouse.Controller()` ??????????????
- ?? WebDAV ?????? `http://` ??????????????????????
- ?? `overlay.py` ? `show_preview` ?? `geometry()` ?? `availableGeometry()`??????? + ?? DPI ?????
- ?? `autostart.py` ? docstring ??????????
- ?? `main.py` ????? Windows `MessageBoxW` ????? `\n` ?????????
- ?? `ApplicationController.start_threads()` ? `ScrollEngine` ??????????????? `MainWindow._start_threads()` ???????????????????
- ?? `ScrollEngine.run()` ? active ???????????????????????????????? CPU ???
- ?? `GlobalInputListener` ???????????????????????????????????????????????

### Improved
- ????????????????? docstring ????`app_controller.py`?`settings_window.py`?`engine.py`?`config.py`?`listeners.py`?`webdav_dialog.py`?`i18n.py`?`single_instance.py`?`overlay.py`?`main.py`
- ????????????????????`test_engine.py`?`test_config.py`?`test_services.py`?`test_keyboard_webdav.py`
- ?? `set_config_attr(name, value)` ????????? UI ?? `cfg` ?????????????????
- `listeners.py` ? `import platform` ?????????????????

### Added
- ?? `tests/test_engine.py` ?????23 ??????? `PowerCurveStrategy` ????? `ScrollEngine` ????
  - `PowerCurveStrategy`?????????????????????????????
  - `ScrollEngine`???????????/???????????????????????????
- ?? `test_config.py` ? `test_to_dict_for_sync_equals_to_dict` ? `test_all_presets_share_common_defaults` ????
- ?? `webdav.insecure_http_warning` ????????
- ?? `tests/test_app_controller.py`??? `ScrollEngine` ????? `start_threads()` ?????
- ? `tests/test_engine.py` ?????? active ???????????? CPU ????
- ? `tests/test_input_listener_timing.py` ???????????????????

### Removed
- ?? `services/crypto.py` ????XOR ?????????????

### Changed
- `flowscroll-homepage` ???????????????????????????????????????????????????????/????
- ?????????????????????????????????????????????? GitHub / Gitee ??
- README ? README.en ????????????????? `https://flowscroll.pages.dev/`
...
### Removed
- ?????? GitHub / Releases / Gitee ?????????????????? manifest / JSON ??????
